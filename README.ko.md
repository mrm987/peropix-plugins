...
- **공통 자산** — 앱이 플러그인과 같은 오리진에 둘을 서빙합니다. 페이지 머리에 두 줄만 넣으면 됩니다:
  ...
  **글꼴도 앱과 같습니다** — 앱이 번들한 글꼴을 같은 자리에서 주고, 사용자가 설정에서 고른 글꼴을 `peropix.js` 가 꽂습니다
  (설정에서 바꾸면 새로고침 없이 따라옵니다). 플러그인이 할 일은 없습니다.
...
- 살아 있는 예: [태그 굴리기](https://github.com/mrm987/peropix-plugin-tag-roll) 의 `index.py`
  (릴리즈에서 13개 파일 960MB 를 받아 sha256 을 대조하고, 창 아래에 「색인 관리」를 둡니다).
