...
3. **[`index.json`](index.json) に項目をひとつ追加する PR を開きます。**
   ...
   | `name` | はい | アプリに表示される名前。文字列一つでも、言語別のオブジェクト（`{"ko": "…", "en": "…", "ja": "…"}`）でも構いません。 |
   | `description` | いいえ | 名前の下に表示される一行。 |
   | `version` | いいえ | 省略するとタグから先頭の `v` を除いた値。インストール済みより高いとアプリが更新を提案します。 |
   | `sha256` | いいえ | 書くと、アプリが取得した zip と照合します。 |

   CI がそのタグの `plugin.json` を取得して `id` が項目と一致するかを確認し、危険に見えるコードパターンを
   一覧にします。統合の前に人もコードを読みます。**チェックが通ったことは安全である意味ではありません** —
   プラグインはアプリと同じ権限で動き、入れるかどうかはユーザーの判断です。統合されると、ユーザーが次に
   管理タブを開いたとき一覧に現れます。

   **`official` は書かないでください。** アプリが「公式」バッジを描くための印なので、PeroPix チームだけが付けます。
   PR に入っていると CI が拒否します。

   **id と名前は一意です。** すでに一覧にある id・名前は
...
my-plugin/
  ...
  _data/             (アプリは作りません) プラグインがダウンロードするもの — 更新しても残ります。下記
```
...
索引・モデル・キャッシュのように**パッケージに入れるには大きいもの**は、プラグイン自身がダウンロードして
`plugins/<id>/_data/` に置きます。新しい版を入れるとき、アプリは旧コピーの `_data/` だけを新コピーへ移します。
だからプラグインは自分のフォルダの外に出ずに、版を上げても大きなファイルを再取得しません。

```
plugins/my-plugin/
  plugin.json          パッケージに含まれます (更新で置き換わります)
  server.py            〃
  _lib/                インストール時に pip が作ります — 版ごとに入れ直されます
  _data/               プラグインが作ります — 版が変わっても引き継がれます
    model.onnx
    index/…
```

- **作るのはプラグインです。** アプリは作りませんし、中も見ません。構造は自由です。
- **パッケージ(タグの圧縮版)に入れないでください。** `_` で始まるフォルダはプラグインとして読まれず、
  リポジトリでは `.gitignore` に入れる場所です。
- **ダウンロードはプラグインの仕事です。** アプリは代わりに取得しません。進捗・検証(サイズ・ハッシュ)・中止・
  削除は自分の画面に用意してください。大きなファイルは GitHub の**リリースアセット**が便利です
  (1ファイル 2GiB 未満・1リリース 1000 個まで・総量と転送量の制限なし)。`Range` による再開もできます。
- **プラグインを削除すると `_data/` もいっしょにゴミ箱へ行きます。** データだけ消す操作が必要なら自分の画面に置いてください。
- 移動に失敗した場合(ファイル使用中など)、データは旧フォルダ `_old-<id>-<時刻>/_data/` に残ります。
- 動く例: [タグロール](https://github.com/mrm987/peropix-plugin-tag-roll) の `index.py`
  (リリースから 13 ファイル 960MB を受け取り sha256 を照合し、フッターに「索引の管理」を置いています)。
